...
class MicroStatements:
	...
	def extract_sub_verb_object(self, sentence):
		"""
		...
		for i in range(len(dependency_tree)):
			...
			if pb == 1:
				if (ch.islower()==True or ch.isdigit()==True or ch==" " or ch==","):
					if( (object_phrase[:-1]!=" " and ch==" ") or ch!=" "):
						object_phrase = object_phrase+ch
			
		self.svo = {"subject":subject.strip(), "verb":verb_phrase.strip(), "object":object_phrase.strip()}


	def split_conj(self, sent_list):
		"""
	...
	def handle_conjunction(self, sentence):
		...
		obj = self.svo["object"].replace(",","and")
	...
	def mwp_split(self, mwp):
		...
		for i in mwp_split_temp:
			temp = self.handle_conjunction(i)
			for j in temp:
				mwp_split.append(j.strip())

		for i, sent in enumerate(mwp_split):
			if "and" in sent:
				res.extend(self.handle_conjunction(sent))
				continue
			res.append(sent)
